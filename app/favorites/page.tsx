...
function getFavoriteAvailability(
  ...
  if (!offer) {
    return {
      ...
      className: "bg-white text-[#6b6558]",
    };
  }
      ...
      <section className="px-4 py-6 sm:px-5 sm:py-8 md:px-12 md:py-14">
        ...
        {loading && (
          <div className="mt-8 premium-card rounded-3xl p-8">
            <p className="font-semibold text-[#6b6558]">
              {t("favorites.loading")}
            </p>
          </div>
        )}
        ...
        {!loading && favorites.length === 0 && !message && (
          <div className="mt-8 premium-card rounded-[1.75rem] p-10 text-center">
            <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-[#ece7da] text-4xl text-[#1a1815]">
              ♥
            </div>
        ...
        {!loading && favorites.length > 0 && (
          <div className="mt-6 premium-muted-card rounded-3xl p-5 sm:p-6">
            <p className="font-semibold text-[#1a1815]">
        ...
        <div className="mt-6 grid gap-5 sm:mt-8 sm:gap-6 md:grid-cols-2 xl:grid-cols-3">
          {favorites.map((favorite) => {
            ...
            return (
              <div
                key={favorite.id}
                className="premium-card overflow-hidden rounded-3xl sm:rounded-[1.75rem]"
              >
                <div
                  className={`relative h-52 overflow-hidden bg-[#ece7da] sm:h-56 ${offer ? "photo-warm-overlay" : ""}`}
                >
                  {offer ? (
                    <OfferImage
                      src={offer.image_url}
                  ...
                  {offer && (
                    <div className="absolute right-4 top-4 rounded-full bg-white/95 px-4 py-2 text-sm font-black text-[#1a1815] shadow-sm ring-1 ring-black/5">
                ...
                <div className="p-4 sm:p-5 md:p-6">
                  ...
                  <div className="mt-4 grid gap-3 font-semibold text-[#6b6558]">
                    <div className="grid gap-3 rounded-3xl bg-[#ece7da] p-4">
                      ...
                      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        {offer?.businesses?.address && (
                          <a
                            ...
                            className="inline-flex min-h-10 w-full items-center justify-center rounded-full bg-white px-4 py-2 text-sm font-black text-[#1a1815] shadow-sm ring-1 ring-black/5 transition hover:text-[#5c7a5c] sm:w-auto"
                          >
                            {t("common.openMap")}
                          </a>
                        )}
                  ...
                  <div className="mt-6 flex flex-col gap-4">
                      ...
                      {offer && (
                        <>
                          ...
                          <div className="mt-1 flex flex-wrap items-end gap-3">
                            <span className="text-4xl font-black text-[#5c7a5c]">
                              {formatMoney(offer.price)}
                            </span>

                            {offer.old_price && (
                              <span className="pb-1 font-medium text-[#6b6558] line-through">
                                {formatMoney(offer.old_price)}
                              </span>
                            )}
                          </div>
                          <p className="mt-2 text-sm font-black text-[#6b6558]">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      <button
                        ...
                        className="min-h-12 rounded-full border border-red-200 bg-red-50 px-5 py-3 font-semibold text-red-700 transition hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-60"
                      >
